...
minetest.register_node( module_name .. ":biofuel_distiller", {
    ...
    can_dig = function(pos,player)
        ...
        if not inv:is_empty("dst") or not inv:is_empty("src") then return false end
    ...
    allow_metadata_inventory_put = function(pos, listname, index, stack, player)
        ...
        if listname == "src" then return stack:get_count()
        elseif listname == "dst" then return 0 end
    end,
    ...
    allow_metadata_inventory_move = function(pos, from_list, from_index, to_list, to_index, count, player)
        ...
        if to_list == "src" then return count
        elseif to_list == "dst" then return 0 end
    end,
    ...
    on_timer = function(pos)
        ...
        local has_item -- does it contain any of the source items on the list?

        for n = 1, #ferment do --normal items
            if inv:contains_item("src", ItemStack(ferment[n][1])) then has_item = n break end
        end
            ...
            if inv_content then
                for k, v in pairs(inv_content) do
                    ...
                    for n = 1, #ferment_groups do
                        if minetest.get_item_group(item_name, ferment_groups[n]) == 1 then has_group = n break end
                    end
                end
            end
        end

        if not has_item and not has_group then return false end

        -- is there room for additional fermentation?
        if has_item and not inv:room_for_item("dst", ferment[has_item][2]) then meta:set_string("infotext", S("Fuel Distiller (FULL)")) return true end

        if has_group and not inv:room_for_item("dst", new_gallon_id) then meta:set_string("infotext", S("Fuel Distiller (FULL)")) return true end
        ...
        if status < 100 then
            ...
            if not has_group then
                ...
                for i,itemstack in pairs(inv:get_list("src")) do inv:remove_item("src", ItemStack(itemstack:get_name().." 1")) end
...
minetest.register_node(":".."biofuel:biofuel_distiller", { groups = {old_biofuel=1}, })
